...
async def create_fruit(session: AsyncSession, request: Request, input: CreateFruitInput) -> UsecaseOutput:
    ...
    if input.template_id:
        ...
        found_templates = await RepoFruitTemplate.get_by_type(session=session, type="NORMAL")
        if not found_templates:
            return UsecaseOutput(
                content={
                    "message": "No NORMAL fruit templates available"
                },
                code=404
            )
        template_id = random.choice(found_templates).id
